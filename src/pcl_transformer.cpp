...
class PclTransformer : public nodelet::Nodelet
{
public:
  ...
  {
    float transform_x, transform_y, transform_z, transform_roll, transform_pitch, transform_yaw;
    getPrivateNodeHandle().getParam("transform_x", transform_x);
    getPrivateNodeHandle().getParam("transform_y", transform_y);
    getPrivateNodeHandle().getParam("transform_z", transform_z);
    getPrivateNodeHandle().getParam("transform_roll", transform_roll);
    getPrivateNodeHandle().getParam("transform_pitch", transform_pitch);
    getPrivateNodeHandle().getParam("transform_yaw", transform_yaw);
    transform_.resize(6);
    transform_[0] = transform_x;
    transform_[1] = transform_y;
    transform_[2] = transform_z;
    transform_[3] = transform_roll;
    transform_[4] = transform_pitch;
    transform_[5] = transform_yaw;
    if (transform_.size()!=6)
    {
      ROS_FATAL("Expected transform of size 6 but got one of size %d. Aborting...", transform_.size());
      return;
    }

    pub_ = getPrivateNodeHandle().advertise<sensor_msgs::PointCloud2>("output", 5);
    sub_ = getPrivateNodeHandle().subscribe("input", 5, &PclTransformer::callback, this);
  };
  ...
  inline void callback(const sensor_msgs::PointCloud2ConstPtr& input)
  {
    ...
    tform_msg.setOrigin(tf::Vector3(transform_[0], transform_[1], transform_[2]));
    tf::Quaternion q;
    ...
    output_msg.header.frame_id = input->header.frame_id;
...
PLUGINLIB_EXPORT_CLASS(pcl_ros_toolbox::PclTransformer, nodelet::Nodelet)
